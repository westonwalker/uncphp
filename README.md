...
The project ships with `unc.css`, a lightweight utility class library, and `unc-extend.css` for customization. Both are imported in `assets/css/app.css`, which is included in every page via the header component.

- **`assets/css/unc.css`** — utility classes and design tokens (spacing, color, typography, etc.). Do not edit this file.
- **`assets/css/unc-extend.css`** — override tokens or add custom component classes here.

This setup is entirely optional. If you prefer a different CSS framework (Tailwind, Bootstrap, Bulma, etc.), replace the imports in `assets/css/app.css` and remove `unc.css` and `unc-extend.css`.
